...
import { WorkflowExecutionPlanPhase,WorkflowExecutionPlan } from "@/types/workflows";
    ...
    for(let phase=2;phase<=nodes.length&&planned.size<nodes.length;phase++){
        ...
        for(const currentNode of nodes){
            if(planned.has(currentNode.id)){
                // Node already put in the exection plan
                continue;
            }
...
function getInvalidInputs(node:AppNode,edges:Edge[],planned:Set<string>){
    ...
    for(const input of inputs){
        ...
        if(requiredInputProvidedByVisitedOutput){
            // the inputs is required and we have a valid value for it provided by a task that is already planned
            continue;
        }
